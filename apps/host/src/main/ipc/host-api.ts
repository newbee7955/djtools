...
export function registerHostIpc(mainWindow: BrowserWindow): void {
  ...
  // 2.1 动态同步侧边栏宽度 (折叠/展开适配)
  ipcMain.handle('host:view:set-sidebar-width', async (_, width: number) => {
    containerManager.setSidebarWidth(width)
    return { success: true }
  })
  ...
  // 2.3 动态同步应用主体/主题外观至沙箱插件
  ipcMain.handle('host:view:set-theme', async (_, theme: string) => {
    containerManager.setTheme(theme)
    return { success: true }
  })
  ...
  ipcMain.handle('host:plugins:install-zip', async () => {
    ...
    try {
      ...
      AppTrayManager.getInstance().rebuildContextMenu()
      return result
    } catch (err: any) {
  ...
  ipcMain.handle('host:app:get-auto-start', () => {
    return AppTrayManager.getInstance().getAutoStart()
  })

  ipcMain.handle('host:app:set-auto-start', (_, enable: boolean) => {
    AppTrayManager.getInstance().updateAutoStart(enable)
    return true
  })

  ipcMain.handle('host:app:open-external', async (_, url: string) => {
    ...
    async (
      ...
      try {
        ...
        const res = await dialog.showSaveDialog(mainWindow, {
          title: '导出豆角全局配置文件',
          defaultPath: `doujiao-backup-${yyyymmdd}.djcfg`,
          filters: [{ name: '豆角加密配置文件 (*.djcfg)', extensions: ['djcfg'] }]
        })
        ...
        const filePath = await ConfigBackupService.getInstance().exportToFile(
          res.filePath,
          password,
          modules,
          appPrefs
        ...
        console.error('[HostApi] 导出配置失败:', err)
        return { success: false, error: err.message || '导出失败' }
      }
    }
  )

  ipcMain.handle('host:config:select-import-file', async () => {
    const res = await dialog.showOpenDialog(mainWindow, {
      title: '选择要导入的豆角配置文件',
      filters: [
        { name: '豆角配置文件 (*.djcfg, *.json)', extensions: ['djcfg', 'json'] },
  ...
  ipcMain.handle(
    'host:config:inspect',
    async (_, { filePath, password }: { filePath: string; password?: string }) => {
      return await ConfigBackupService.getInstance().inspectBackupFile(filePath, password)
    }
  )
  ...
  ipcMain.handle(
    ...
    async (
      ...
      return await ConfigBackupService.getInstance().importFromFile(filePath, password, options)
    }
  )
}
