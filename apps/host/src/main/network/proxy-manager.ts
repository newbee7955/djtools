...
const DEFAULT_CONFIG: NetworkProxyConfig = {
  ...
  bypassRules:
    '<local>;localhost;127.0.0.1;192.168.*;10.*;172.16.*;172.17.*;172.18.*;172.19.*;172.20.*;172.21.*;172.22.*;172.23.*;172.24.*;172.25.*;172.26.*;172.27.*;172.28.*;172.29.*;172.30.*;172.31.*'
}
...
export class ProxyManager {
  ...
  private loadConfig(): void {
    if (existsSync(this.configFile)) {
      try {
        ...
        // 自动升级旧版缺失局域网网段的 bypassRules
        if (!this.config.bypassRules || this.config.bypassRules === '<local>;localhost;127.0.0.1') {
          this.config.bypassRules = DEFAULT_CONFIG.bypassRules
          this.saveConfig()
        }
      } catch (err) {
  ...
  public async init(): Promise<void> {
    ...
    // 异步解析生效代理仅用于输出日志，严禁阻塞首屏窗口的创建与显示
    void this.resolveProxy('https://github.com').then((effective) => {
  ...
  public async applyProxy(): Promise<void> {
    try {
      const bypass = this.config.bypassRules || DEFAULT_CONFIG.bypassRules
      if (this.config.mode === 'direct') {
        ...
        // 默认：跟随系统代理 (system)，显式传入局域网与私有 IP 绕过规则
        await session.defaultSession.setProxy({
          mode: 'system',
          proxyBypassRules: bypass
        })
        console.log('[ProxyManager] Switched to system proxy mode (with LAN bypass)')
      }
    } catch (err) {
  ...
  public async testGitHubConnectivity(): Promise<{
    ...
    const testUrl = 'https://raw.githubusercontent.com/newbee7955/djtools/main/registry/plugins-registry.json'
    const effective = await this.resolveProxy('https://github.com')
