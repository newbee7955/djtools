...
import { toGlobalRecordingBounds } from './screen-recording-core'
...
export class ScreenshotService {
  ...
  /**
   * Reuse the screenshot overlay as a bounds picker for screen recording.
   * This mode never writes the clipboard or notifies the active plugin.
   */
  public async selectRegion(): Promise<ScreenCaptureResult> {
    return this.capture({ hideWindow: true, mode: 'snip', selectionOnly: true })
  }
  ...
  private registerIpcHandlers(): void {
    ...
    ipcMain.handle(
      ...
      async (_, payload: { dataUrl: string; bounds?: { x: number; y: number; width: number; height: number } }) => {
        ...
        const captureData = this.currentCaptureData
        const selectionOnly = captureData?.selectionOnly === true
        const resolvedBounds =
          selectionOnly && bounds && captureData
            ? toGlobalRecordingBounds(bounds, {
                x: captureData.displayX,
                y: captureData.displayY
              })
            : bounds

        // 1. 自动存入系统剪贴板
        if (!selectionOnly && dataUrl) {
        ...
        const result: ScreenCaptureResult = {
          success: true,
          dataUrl: selectionOnly ? undefined : dataUrl,
    ...
    ipcMain.handle(
      ...
      async (_, payload: { dataUrl: string; bounds?: { x: number; y: number; width: number; height: number } }) => {
        ...
        if (this.pendingResolve) {
          const cb = this.pendingResolve
          this.pendingResolve = null
          cb({ success: true, dataUrl, bounds })
        }

        if (dataUrl) {
          const { PinManager } = await import('../container/pin-manager')
          PinManager.getInstance().createPin({
            dataUrl,
            bounds
          })
        }
    ...
    // 复制吸取的颜色字符串
    ipcMain.handle('host:screenshot:copy-color', async (_, text: string) => {
      if (text) {
...
<body>
  <div id="guide-banner">
    <span>拖拽鼠标划选截图</span>
    <span class="kbd">C 吸色HEX</span>
    <span class="kbd">Shift+C 吸色RGB</span>
    <span class="kbd">F3 贴图</span>
  ...
  <div id="toolbar">
    ...
    <button class="btn btn-secondary" id="btn-pin" title="贴到屏幕 (F3)">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
  ...
  <script>
      ...
      try {
        if (window.screenshotAPI?.getInitialData) {
          ...
          if (initData && initData.bgDataUrl) {
            ...
            selectionOnly = initData.selectionOnly === true;
          }
        }
      } catch (err) {
    ...
    if (selectionOnly) {
      guideBanner.innerHTML = '<span>拖拽鼠标选择录制区域</span><span class="kbd">Enter 开始录制</span><span class="kbd">Space 全屏</span><span class="kbd">Esc 取消</span>';
      ['btn-pin', 'btn-save', 'btn-copy'].forEach((id) => {
        const button = document.getElementById(id);
        if (button) button.style.display = 'none';
      });
      const finishButton = document.getElementById('btn-finish');
      finishButton.textContent = '开始录制';
      finishButton.title = '确认录制区域 (Enter)';
    }
    ...
    function enterEditMode() {
      phase = 'edit';
      annoCanvas.style.pointerEvents = selectionOnly ? 'none' : 'auto';
      document.body.style.cursor = 'crosshair';
      guideBanner.style.display = 'none';
      annoToolbar.style.display = selectionOnly ? 'none' : 'flex';
      toolbar.style.display = 'flex';
      if (!selectionOnly) updateAnnoToolbarPos();
      updateToolbarPos();
    ...
    let currentSampledHex = '#000000';
    let currentSampledRgb = 'rgb(0,0,0)';
    ...
    function showToast(msg) {
      ...
      if (!toast) {
        ...
        toast.id = 'cap-toast';
        toast.style.cssText = 'position:fixed;bottom:24px;left:50%;transform:translateX(-50%);padding:7px 18px;background:rgba(15,23,42,0.92);color:#fff;border-radius:20px;font-size:12px;font-weight:600;box-shadow:0 8px 24px rgba(0,0,0,0.3);z-index:9999;pointer-events:none;transition:opacity .2s;backdrop-filter:blur(8px);border:1px solid rgba(255,255,255,0.15);';
    ...
    function updateMagnifier(mx, my) {
      ...
      try {
        ...
        currentSampledHex = hex;
        currentSampledRgb = 'rgb('+px[0]+','+px[1]+','+px[2]+')';
    ...
    function doFinish() {
      commitTextIfActive();
      const dataUrl = selectionOnly ? '' : (getCroppedDataUrl() || bgDataUrl);
    ...
    function doPin() {
      ...
      if (window.screenshotAPI?.pin) {
        window.screenshotAPI.pin({
          dataUrl: d,
          bounds: cropRect.width > 0 ? { x:cropRect.x, y:cropRect.y, width:cropRect.width, height:cropRect.height } : undefined
        });
      } else {
        window.close();
      }
    ...
    document.getElementById('btn-pin').onclick        = doPin;
    ...
    window.addEventListener('keydown', (e) => {
      ...
      if ((e.key === 'c' || e.key === 'C') && !e.ctrlKey && !e.metaKey) {
        if (phase === 'select' && magnifier.style.display !== 'none') {
          e.preventDefault();
          if (e.shiftKey) {
            if (window.screenshotAPI?.copyColor) {
              window.screenshotAPI.copyColor(currentSampledRgb);
            }
            showToast('已复制 RGB: ' + currentSampledRgb);
          } else {
            if (window.screenshotAPI?.copyColor) {
              window.screenshotAPI.copyColor(currentSampledHex);
            }
            showToast('已复制 HEX: ' + currentSampledHex);
          }
          return;
        }
      }
