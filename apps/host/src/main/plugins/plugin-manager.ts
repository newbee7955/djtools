...
export class PluginManager {
  ...
  public isPluginAvailable(pluginId: string): boolean {
    ...
    return Boolean(plugin && plugin.enabled !== false)
  }
  ...
  public listAllPlugins(): DiscoveredPlugin[] {
    ...
    // 1. 在开发环境 (!app.isPackaged) 或显式声明 DOUJIAO_DEV_PLUGINS=true 时自动探测源码 plugins/* 目录
    if (!app.isPackaged || process.env.DOUJIAO_DEV_PLUGINS === 'true') {
      try {
        ...
        for (const dirent of dirs) {
          ...
          if (existsSync(manifestFile)) {
            try {
              ...
              const manifest: PluginManifest = JSON.parse(raw.replace(/^\uFEFF/, ''))
              const distDir = join(versionDir, 'dist')
