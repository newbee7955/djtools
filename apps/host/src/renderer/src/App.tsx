...
export default function App(): JSX.Element {
  ...
  const [importStep, setImportStep] = useState<'password' | 'preview'>('password')
  const [importInspectData, setImportInspectData] = useState<any | null>(null)
  ...
  useEffect(() => {
    ...
    window.hostAPI?.getAutoStart?.().then((val: boolean) => {
  ...
  const handleDoExport = async () => {
    if (!exportPassword || exportPassword.length < 6) {
      setBackupMsg({ text: '加密密码长度不能少于 6 位', type: 'error' })
      return
    }
    if (exportPassword !== exportConfirmPassword) {
      setBackupMsg({ text: '两次输入的加密密码不一致', type: 'error' })
      return
    }
    ...
    try {
      ...
      const res = await window.hostAPI?.exportConfig?.({
      ...
      if (res?.success) {
        ...
        setBackupMsg({ text: `✓ 配置已成功加密导出！文件保存至: ${res.filePath}`, type: 'success' })
      } else {
        setBackupMsg({ text: res?.error || '导出配置失败', type: 'error' })
      }
    } catch (err: any) {
      setBackupMsg({ text: err.message || '导出异常', type: 'error' })
    } finally {
      setExportLoading(false)
    }
  }
  ...
  const handleStartImport = async () => {
    try {
      const res = await window.hostAPI?.selectImportFile?.()
      if (res?.canceled || !res?.filePath) return
      setImportFilePath(res.filePath)
      setImportPassword('')
      setImportLoading(true)
      const inspect = await window.hostAPI?.inspectBackup?.({ filePath: res.filePath })
      setImportLoading(false)
      if (!inspect?.valid) {
        setBackupMsg({ text: inspect?.error || '无效的备份文件', type: 'error' })
        return
      }
      setImportInspectData(inspect)
      setImportModules({
        samba: (inspect.summary?.sambaCount || 0) > 0,
        objectStorage: (inspect.summary?.objectStorageCount || 0) > 0,
        proxy: !!inspect.summary?.hasProxy,
        workspaces: !!inspect.summary?.hasWorkspaces,
        lanTransfer: !!inspect.summary?.hasLanTransfer,
        appPrefs: !!inspect.summary?.hasAppPrefs
      })
      setImportStep('password')
      setShowImportModal(true)
    } catch (err: any) {
      setBackupMsg({ text: err.message || '选择备份文件失败', type: 'error' })
    }
  }

  const handleVerifyImportPassword = async () => {
    if (!importPassword) {
      setBackupMsg({ text: '请输入解密密码', type: 'error' })
      return
    }
    setImportLoading(true)
    try {
      const inspect = await window.hostAPI?.inspectBackup?.({
        filePath: importFilePath,
        password: importPassword
      })
      if (!inspect?.valid || !inspect.decryptedData) {
        setBackupMsg({ text: inspect?.error || '解密失败，密码错误或文件已损坏', type: 'error' })
        setImportLoading(false)
        return
      }
      setImportInspectData(inspect)
      ...
      setBackupMsg({ text: err.message || '密码校验异常', type: 'error' })
    } finally {
      setImportLoading(false)
    }
  }
  ...
  const handleDoImport = async () => {
    setImportLoading(true)
    try {
      const res = await window.hostAPI?.importConfig?.({
        filePath: importFilePath,
        password: importPassword,
        options: {
          mode: importMode,
      ...
      if (res?.success) {
        ...
        if (res.details?.appPrefs) {
          const p = res.details.appPrefs
          if (p.theme && THEMES[p.theme as AppTheme]) {
            handleSetTheme(p.theme as AppTheme)
          }
          if (p.pinnedPluginIds && Array.isArray(p.pinnedPluginIds)) {
            setPinnedPluginIds(p.pinnedPluginIds)
            try {
              localStorage.setItem('doujiao_pinned_plugins', JSON.stringify(p.pinnedPluginIds))
            } catch {}
          }
          ...
          if (typeof p.autoCheckUpdate === 'boolean') {
            setAutoCheckUpdate(p.autoCheckUpdate)
          }
        }
        if (window.hostAPI?.getProxyStatus) {
          window.hostAPI.getProxyStatus().then((st: any) => {
            if (st) setProxyConfig(st)
          })
        }
        if (window.hostAPI?.getWorkspaceDirectories) {
          window.hostAPI.getWorkspaceDirectories().then((ws: any) => {
            if (ws) setWorkspaces(ws)
          })
        }
        setBackupMsg({
          text: `✓ 配置恢复成功！已导入模块: ${res.importedModules.join(', ')}`,
          type: 'success'
        })
      } else {
        setBackupMsg({ text: res?.error || '导入配置失败', type: 'error' })
      }
    } catch (err: any) {
      setBackupMsg({ text: err.message || '导入配置异常', type: 'error' })
    } finally {
      setImportLoading(false)
    }
  }
  ...
  const handleToggleAutoStart = async (enabled: boolean) => {
    ...
    try {
      await window.hostAPI?.setAutoStart?.(enabled)
    } catch (err) {
    ...
    <div className={`flex flex-col h-screen w-screen overflow-hidden ${currentTheme.appBg} ${currentTheme.textPrimary} select-none transition-colors duration-200`}>
      ...
      >
        ...
        >
          ...
          <button
            onClick={() => {
              const url = 'https://github.com/newbee7955/djtools'
              if (window.hostAPI?.openExternal) {
                window.hostAPI.openExternal(url)
              } else {
            ...
            title="访问 GitHub 源码仓库 (https://github.com/newbee7955/djtools)"
          >
              ...
              <div className={`p-5 rounded-xl border space-y-5 ${currentTheme.cardBg} ${currentTheme.border}`}>
                  ...
                  <div className={`flex items-center justify-between p-3 rounded-xl ${currentTheme.subcardBg} border ${currentTheme.border}`}>
                    <div className="flex items-center gap-3">
                      <input
                        ...
                        className={`w-4 h-4 rounded text-indigo-600 focus:ring-0 ${theme === 'light' ? 'bg-white border-slate-300' : 'bg-slate-900 border-slate-700'} cursor-pointer`}
                      />
                      <label htmlFor="autoStartApp" className="cursor-pointer">
                        <div className={`text-xs font-semibold ${currentTheme.textPrimary}`}>开机时自动启动应用</div>
                        <div className={`text-[10px] ${currentTheme.textMuted}`}>开启后电脑开机自动静默启动并常驻系统托盘，随时响应远程协助、桌面贴图与全局快捷键</div>
                      </label>
                    </div>
                  </div>
                ...
                <div className={`border-t ${currentTheme.border} pt-4 space-y-3`}>
                  <div className="flex items-start justify-between">
                    <div>
                      <div className={`text-sm font-medium ${currentTheme.textPrimary} flex items-center gap-2`}>
                        <span>全局配置备份与迁移 (导出 / 导入)</span>
                        <span className="text-[10px] px-2 py-0.5 rounded-full font-mono bg-indigo-500/10 text-indigo-500 border border-indigo-500/30">
                          AES-256-GCM 强加密
                        </span>
                      </div>
                      <div className={`text-xs ${currentTheme.textMuted} mt-0.5`}>
                        一键导出/导入 Samba 连接、对象存储连接、网络代理、工作目录及通用偏好。密码与密钥绝不以明文存储，保障换机迁移与灾备安全。
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <button
                        ...
                        className="px-3.5 py-1.5 text-xs rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white font-medium shadow-md shadow-indigo-600/20 transition-all flex items-center gap-1.5"
                      >
                        <span>📤 导出配置</span>
                      </button>
                      <button
                        onClick={handleStartImport}
                        className={`px-3.5 py-1.5 text-xs rounded-lg ${currentTheme.btnGhost} transition-all flex items-center gap-1.5`}
                      >
                        <span>📥 导入配置</span>
                      </button>
                    </div>
                  ...
                  {backupMsg && (
                    ...
                    >
                      ...
                      <button onClick={() => setBackupMsg(null)} className={`${currentTheme.textMuted} ${currentTheme.hoverText}`}>✕</button>
                    </div>
                  )}
                </div>
          ...
          <div className={`${currentTheme.cardBg} border ${currentTheme.border} rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-4`}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <span className="text-2xl">📤</span>
                <div>
                  <h3 className={`font-bold ${currentTheme.textPrimary} text-base`}>导出全局配置备份</h3>
                  <p className={`text-xs ${currentTheme.textMuted} mt-0.5`}>
                    选择需要导出的配置项并设置强加密密码
                  </p>
                </div>
              ...
              <button
                ...
                className={`text-sm ${currentTheme.textMuted} ${currentTheme.hoverText}`}
              >
                ✕
              </button>
            </div>
            ...
            <div className="space-y-2">
              <div className={`text-xs font-semibold ${currentTheme.textPrimary}`}>1. 选择导出的配置模块</div>
              <div className={`grid grid-cols-2 gap-2 p-3 rounded-xl ${currentTheme.subcardBg} border ${currentTheme.border}`}>
                <label className="flex items-center gap-2 cursor-pointer text-xs">
                  <input
                    type="checkbox"
                    checked={exportModules.samba}
                    onChange={(e) => setExportModules({ ...exportModules, samba: e.target.checked })}
                    className="text-indigo-600 rounded"
                  />
                  <span className={currentTheme.textPrimary}>🗄️ Samba 连接配置</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer text-xs">
                  <input
                    type="checkbox"
                    checked={exportModules.objectStorage}
                    onChange={(e) => setExportModules({ ...exportModules, objectStorage: e.target.checked })}
                    className="text-indigo-600 rounded"
                  />
                  <span className={currentTheme.textPrimary}>🪣 对象存储配置</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer text-xs">
                  <input
                    type="checkbox"
                    checked={exportModules.proxy}
                    onChange={(e) => setExportModules({ ...exportModules, proxy: e.target.checked })}
                    className="text-indigo-600 rounded"
                  />
                  <span className={currentTheme.textPrimary}>🌐 网络代理配置</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer text-xs">
                  <input
                    type="checkbox"
                    checked={exportModules.workspaces}
                    onChange={(e) => setExportModules({ ...exportModules, workspaces: e.target.checked })}
                    className="text-indigo-600 rounded"
                  />
                  <span className={currentTheme.textPrimary}>📁 文件存储与工作目录</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer text-xs">
                  <input
                    type="checkbox"
                    checked={exportModules.lanTransfer}
                    onChange={(e) => setExportModules({ ...exportModules, lanTransfer: e.target.checked })}
                    className="text-indigo-600 rounded"
                  />
                  <span className={currentTheme.textPrimary}>📱 局域网互传设置</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer text-xs">
                  <input
                    type="checkbox"
                    checked={exportModules.appPrefs}
                    onChange={(e) => setExportModules({ ...exportModules, appPrefs: e.target.checked })}
                    className="text-indigo-600 rounded"
                  />
                  <span className={currentTheme.textPrimary}>🎨 应用偏好与主题</span>
                </label>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <div className={`text-xs font-semibold ${currentTheme.textPrimary}`}>2. 设置安全加密密码 (必填)</div>
                <button
                  type="button"
                  onClick={() => setExportShowPass(!exportShowPass)}
                  className={`text-[11px] ${currentTheme.textMuted} hover:text-indigo-400`}
                >
                  {exportShowPass ? '隐藏密码' : '显示密码'}
                </button>
              </div>
              ...
              <div className="space-y-2">
                <input
                  ...
                  className={`w-full px-3 py-2 text-xs rounded-xl border ${theme === 'light' ? 'bg-white border-slate-200 text-slate-900' : 'bg-slate-900 border-slate-700 text-white'} focus:border-indigo-500 focus:outline-none`}
                />
                <input
                  type={exportShowPass ? 'text' : 'password'}
                  value={exportConfirmPassword}
                  onChange={(e) => setExportConfirmPassword(e.target.value)}
                  placeholder="再次确认加密密码"
                  className={`w-full px-3 py-2 text-xs rounded-xl border ${theme === 'light' ? 'bg-white border-slate-200 text-slate-900' : 'bg-slate-900 border-slate-700 text-white'} focus:border-indigo-500 focus:outline-none`}
                />
              </div>

              <div className="p-2.5 rounded-xl bg-indigo-500/10 border border-indigo-500/30 text-[11px] text-indigo-400 flex items-start gap-2">
                <span>🛡️</span>
                <span>所有 Samba 登录密码与对象存储 SecretAccessKey 将采用 <strong>AES-256-GCM</strong> 工业级加密。导出的备份文件中绝无任何明文凭证，请妥善保管此密码。</span>
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-2.5 pt-2">
              <button
                onClick={() => setShowExportModal(false)}
                className={`px-4 py-2 text-xs rounded-xl ${currentTheme.btnGhost}`}
              >
                取消
              </button>
              <button
                onClick={handleDoExport}
                disabled={exportLoading || !exportPassword || exportPassword.length < 6}
                className="px-5 py-2 text-xs rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white font-semibold shadow-lg shadow-indigo-600/20 transition-all flex items-center gap-1.5"
              >
                <span>{exportLoading ? '⏳ 正在加密导出...' : '⚡ 开始加密导出'}</span>
              </button>
            </div>
          </div>
        </div>
          ...
          <div className={`${currentTheme.cardBg} border ${currentTheme.border} rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-4`}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <span className="text-2xl">📥</span>
                <div>
                  <h3 className={`font-bold ${currentTheme.textPrimary} text-base`}>
                    {importStep === 'password' ? '输入解密密码' : '确认导入配置项'}
                  </h3>
                  <p className={`text-xs ${currentTheme.textMuted} mt-0.5 truncate max-w-sm`} title={importFilePath}>
                    {importFilePath ? `文件: ${importFilePath.split(/[\\/]/).pop()}` : '恢复豆角配置'}
                  </p>
                </div>
              </div>
              <button
            ...
            {importStep === 'password' && (
              <div className="space-y-3">
                <div className={`p-3 rounded-xl ${currentTheme.subcardBg} border ${currentTheme.border} text-xs space-y-1`}>
                  <div className="flex justify-between text-[11px]">
                    <span className={currentTheme.textMuted}>备份时间:</span>
                    <span className={currentTheme.textPrimary}>{importInspectData?.exportedAt ? new Date(importInspectData.exportedAt).toLocaleString() : '未知'}</span>
                  </div>
                  <div className="flex justify-between text-[11px]">
                    <span className={currentTheme.textMuted}>应用版本:</span>
                    <span className={currentTheme.textPrimary}>v{importInspectData?.appVersion || '1.0.0'}</span>
                  </div>
                  <div className="flex justify-between text-[11px]">
                    <span className={currentTheme.textMuted}>加密算法:</span>
                    <span className="font-mono text-emerald-500">AES-256-GCM (PBKDF2)</span>
                  </div>
                </div>

                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    <label className={`text-xs font-semibold ${currentTheme.textPrimary}`}>请输入备份加密密码</label>
                    <button
                      type="button"
                      onClick={() => setImportShowPass(!importShowPass)}
                      className={`text-[11px] ${currentTheme.textMuted} hover:text-indigo-400`}
                    >
                      {importShowPass ? '隐藏密码' : '显示密码'}
                    </button>
                  </div>
                  <input
                    ...
                    onKeyDown={(e) => e.key === 'Enter' && handleVerifyImportPassword()}
                ...
                <div className="flex items-center justify-end gap-2.5 pt-2">
                  ...
                  <button
                    onClick={handleVerifyImportPassword}
                    disabled={importLoading || !importPassword}
                    className="px-5 py-2 text-xs rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white font-semibold shadow-lg shadow-indigo-600/20 transition-all flex items-center gap-1.5"
                  >
                    <span>{importLoading ? '⏳ 正在解密验证...' : '🔓 解密并预览'}</span>
                  </button>
                </div>
              </div>
            )}
            ...
            {importStep === 'preview' && (
              <div className="space-y-3">
                {/* 包含的内容预览 */}
                <div className="space-y-1.5">
                  <div className={`text-xs font-semibold ${currentTheme.textPrimary}`}>备份包含的配置清单</div>
                  <div className={`p-3 rounded-xl ${currentTheme.subcardBg} border ${currentTheme.border} space-y-2 text-xs`}>
                    <label className="flex items-center justify-between cursor-pointer">
                      <div className="flex items-center gap-2">
                        <input
                          type="checkbox"
                          checked={importModules.samba}
                          onChange={(e) => setImportModules({ ...importModules, samba: e.target.checked })}
                          className="text-indigo-600 rounded"
                        />
                        <span className={currentTheme.textPrimary}>🗄️ Samba 连接配置</span>
                      </div>
                      <span className="font-mono text-[11px] text-indigo-400">
                        {importInspectData?.summary?.sambaCount || 0} 个连接
                      </span>
                    </label>
                    ...
                    <label className="flex items-center justify-between cursor-pointer">
                      <div className="flex items-center gap-2">
                        <input
                          type="checkbox"
                          checked={importModules.objectStorage}
                          onChange={(e) => setImportModules({ ...importModules, objectStorage: e.target.checked })}
                          className="text-indigo-600 rounded"
                        />
                        <span className={currentTheme.textPrimary}>🪣 对象存储配置</span>
                      </div>
                      <span className="font-mono text-[11px] text-indigo-400">
                        {importInspectData?.summary?.objectStorageCount || 0} 个存储
                      </span>
                    </label>
                    ...
                    <label className="flex items-center justify-between cursor-pointer">
                      <div className="flex items-center gap-2">
                        <input
                          type="checkbox"
                          checked={importModules.proxy}
                          onChange={(e) => setImportModules({ ...importModules, proxy: e.target.checked })}
                          className="text-indigo-600 rounded"
                        />
                        <span className={currentTheme.textPrimary}>🌐 网络代理配置</span>
                      </div>
                      <span className="text-[11px] text-emerald-500">包含</span>
                    </label>
                    ...
                    <label className="flex items-center justify-between cursor-pointer">
                      <div className="flex items-center gap-2">
                        <input
                          type="checkbox"
                          checked={importModules.workspaces}
                          onChange={(e) => setImportModules({ ...importModules, workspaces: e.target.checked })}
                          className="text-indigo-600 rounded"
                        />
                        <span className={currentTheme.textPrimary}>📁 工作目录配置</span>
                      </div>
                      <span className="text-[11px] text-emerald-500">包含</span>
                    </label>
                    ...
                    <label className="flex items-center justify-between cursor-pointer">
                      <div className="flex items-center gap-2">
                        <input
                          type="checkbox"
                          checked={importModules.lanTransfer}
                          onChange={(e) => setImportModules({ ...importModules, lanTransfer: e.target.checked })}
                          className="text-indigo-600 rounded"
                        />
                        <span className={currentTheme.textPrimary}>📱 局域网互传设置</span>
                      </div>
                    ...
                    <label className="flex items-center justify-between cursor-pointer">
                      <div className="flex items-center gap-2">
                        <input
                          type="checkbox"
                          checked={importModules.appPrefs}
                          onChange={(e) => setImportModules({ ...importModules, appPrefs: e.target.checked })}
                          className="text-indigo-600 rounded"
                        />
                        <span className={currentTheme.textPrimary}>🎨 应用通用偏好</span>
                      </div>
                      <span className="text-[11px] text-emerald-500">包含</span>
                    </label>
                  </div>
                ...
                {/* 导入模式选择 */}
                <div className="space-y-1.5">
                  <div className={`text-xs font-semibold ${currentTheme.textPrimary}`}>导入方式</div>
                  <div className="grid grid-cols-2 gap-2">
                    <label
                      ...
                      className={`p-2.5 rounded-xl border cursor-pointer transition-all flex items-center gap-2 ${
                        importMode === 'merge'
                          ...
                          : `${currentTheme.subcardBg} ${currentTheme.border} ${currentTheme.textMuted}`
                      }`}
                    >
                      <input
                        type="radio"
                      ...
                      />
                      <div>
                        <div className={`text-xs font-semibold ${currentTheme.textPrimary}`}>智能合并 (推荐)</div>
                        <div className={`text-[10px] ${currentTheme.textMuted}`}>保留现有配置，冲突项覆盖更新</div>
                      </div>
                    </label>
                    ...
                    >
                      ...
                      />
                      <div>
                        <div className={`text-xs font-semibold ${currentTheme.textPrimary}`}>完全覆盖</div>
                        <div className={`text-[10px] ${currentTheme.textMuted}`}>清空对应模块现有数据后导入</div>
                      </div>
                    </label>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-2">
                  <button
                    ...
                    className={`px-3 py-1.5 text-xs rounded-xl ${currentTheme.btnGhost}`}
                  >
                    ← 重新输入密码
                  </button>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="px-5 py-2 text-xs rounded-xl bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white font-semibold shadow-lg shadow-emerald-600/20 transition-all flex items-center gap-1.5"
                    >
                      <span>{importLoading ? '⏳ 正在恢复配置...' : '✓ 确认导入并生效'}</span>
                    </button>
                  </div>
                </div>
              </div>
