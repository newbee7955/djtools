...
const hostAPI = {
  ...
  openExternal: (url: string) => ipcRenderer.invoke('host:app:open-external', url),
  getAutoStart: () => ipcRenderer.invoke('host:app:get-auto-start'),
  setAutoStart: (enable: boolean) => ipcRenderer.invoke('host:app:set-auto-start', enable),
  ...
  toggleShelf: (visible?: boolean) => ipcRenderer.invoke('plugin:shelf:toggle-window', visible),

  // 全局配置备份与迁移
  exportConfig: (params: { password: string; modules?: any; appPrefs?: any }) =>
    ipcRenderer.invoke('host:config:export', params),
  selectImportFile: () => ipcRenderer.invoke('host:config:select-import-file'),
  inspectBackup: (params: { filePath: string; password?: string }) =>
    ipcRenderer.invoke('host:config:inspect', params),
  importConfig: (params: { filePath: string; password: string; options: any }) =>
    ipcRenderer.invoke('host:config:import', params)
}
