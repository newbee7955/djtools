...
cd /d "%~dp0"
taskkill /F /IM doujiao-dxgi-capturer.exe >nul 2>&1
if not exist "bin" mkdir "bin"

echo [Build] Compiling doujiao-dxgi-capturer.exe...
cl.exe /O2 /W4 /utf-8 /std:c++17 /EHsc src/main.cpp /Fe:bin/doujiao-dxgi-capturer.exe d3d11.lib dxgi.lib user32.lib gdi32.lib ws2_32.lib advapi32.lib
...
bin\doujiao-dxgi-capturer.exe --self-test
if %ERRORLEVEL% NEQ 0 (
    echo [Error] Self-test failed with code %ERRORLEVEL%
    exit /b %ERRORLEVEL%
)

echo [Build] Successfully built and tested doujiao-dxgi-capturer.exe!
exit /b 0
