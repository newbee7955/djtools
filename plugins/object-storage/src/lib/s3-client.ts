  ...
  private async executeRequest(options: {
    ...
    if (hostSdk && typeof hostSdk.request === 'function') {
      try {
        ...
        console.warn('[S3Client] 宿主网络代理请求失败，尝试本地 fetch 降级:', hostErr)
        try {
...
<Delete>
  ...
  async testConnection(): Promise<{ success: boolean; message?: string }> {
    try {
      ...
      let message = err.message || '连接失败，请核对密钥与 Endpoint'
      if (
        (message.includes('ERR_CONNECTION_CLOSED') ||
          message.includes('ECONNRESET') ||
          message.includes('对端重置') ||
          message.includes('关闭')) &&
        this.profile.useSSL !== false
      ) {
        message +=
          '\n💡 诊断提示：如果您的 MinIO 或私有存储使用的是 HTTP 协议（如默认端口 9000），请在下方【高级选项】中取消勾选【使用 HTTPS 安全连接】。'
      }
